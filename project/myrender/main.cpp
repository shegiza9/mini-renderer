...
void set_tangent(Triangle* t) {
	Vec4f tangent;

	Vec4f edge1 = t->v[1].pos - t->v[0].pos;
	Vec4f edge2 = t->v[2].pos - t->v[0].pos;

	float deltau1 = t->v[1].tc.u - t->v[0].tc.u;
	float deltau2 = t->v[2].tc.u - t->v[0].tc.u;
	float deltav1 = t->v[1].tc.v - t->v[0].tc.v;
	float deltav2 = t->v[2].tc.v - t->v[0].tc.v;
	...
	*/
	tangent.normalized();
	t->v[0].tangent = tangent;
	t->v[1].tangent = tangent;
	t->v[2].tangent = tangent;
}
...
int main(int argc, char* argv[]) {
	...
	up = { 0,1,0,1 };
	float theta = 1.25;
	Vec4f ll1 = { x_cent-1.0f,max_y+1.0f,max_z+0.5f,1.0f };
	...
	Vertex_t vlis[8] = {
		{{ll1.x + 0.5f,ll1.y + 0.5f,ll1.z + 0.5f,1.0f},{1,0},{1.0f,1.0f,1.0f},{0,0,1,1},{0,0,0,1},1.0f},
		{{ll1.x + 0.5f,ll1.y + 0.5f,ll1.z - 0.5f,1.0f},{1,0},{1.0f,1.0f,1.0f},{0,0,1,1},{0,0,0,1},1.0f},
		{{ll1.x + 0.5f,ll1.y - 0.5f,ll1.z + 0.5f,1.0f},{1,0},{1.0f,1.0f,1.0f},{0,0,1,1},{0,0,0,1},1.0f},
		{{ll1.x + 0.5f,ll1.y - 0.5f,ll1.z - 0.5f,1.0f},{1,0},{1.0f,1.0f,1.0f},{0,0,1,1},{0,0,0,1},1.0f},
		{{ll1.x - 0.5f,ll1.y + 0.5f,ll1.z + 0.5f,1.0f},{1,0},{1.0f,1.0f,1.0f},{0,0,1,1},{0,0,0,1},1.0f},
	...
	Vertex_t floor_m[4] = {
		{{min_x - 20.0f,min_y - 0.05f,max_z + 20.0f,1.0f},{0,0},{1,1,1},{0,1,0,1},{0,0,0,1},1.0f},
		{{max_x + 20.0f,min_y - 0.05f,max_z + 20.0f,1.0f},{0,0},{1,1,1},{0,1,0,1},{0,0,0,1},1.0f},
		{{min_x - 20.0f,min_y - 0.05f,min_z - 20.0f,1.0f},{0,0},{1,1,1},{0,1,0,1},{0,0,0,1},1.0f},
		{{max_x + 20.0f,min_y - 0.05f,min_z - 20.0f,1.0f},{0,0},{1,1,1},{0,1,0,1},{0,0,0,1},1.0f}
	};
	...
	for (int i = 0; i < mesh_list.size(); ++i) {
		for (int j = 0; j < mesh_list[i]->triangle_list.size(); ++j) {
			...
			mesh_list[i]->triangle_list[j] = NULL;
		}
		delete mesh_list[i];
		mesh_list[i] = NULL;
	}
